...
public class AprendizDao implements MeAprendiz {
    ...
    public boolean actualizarape(Aprendiz pro) {
        String sql = "update aprendiz set Docu='"+pro.getDocu()+"',Nombrea='"+pro.getNombrea()+"',Apellidoa='"+pro.getApellidoa()+"',Correoa='"+pro.getCorreoa()+"',Numcontacto='"+pro.getNumcontacto()+"' where id="+pro.getId();
        try{
            con=cn.getConnection();
            ps=con.prepareStatement(sql);
            ps.executeUpdate();
